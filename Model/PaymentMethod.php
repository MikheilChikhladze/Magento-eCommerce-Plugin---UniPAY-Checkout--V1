...
class PaymentMethod extends AbstractMethod
{
    ...
    private function item($order)
    {
        ...
        $backLink = $this->getBackLink($orderId);

        $arr = [
            "MerchantID"       => $this->getMerchantId(),
            "MerchantUser"     => $customerId,
            "MerchantOrderID"  => $orderId,
            "OrderPrice"       => $orderTotal * 100,
            "OrderCurrency"    => 'GEL',
            "BackLink"         => $backLink,
        ...
        $hash = ['Hash'=> md5($this->getSecretKey().'|'.implode('|', $arr))];
    ...
    private function parseItems($items, $order)
    {
        ...
        foreach ($items as $item) {
            ...
            $itemsArray[] = [
                'price'              => ($product->getFinalPrice() * 100 ) * (int)$item->getQtyOrdered(),
                'quantity'           => (int)$item->getQtyOrdered(),
                'productName'        => $product->getName(),
                'productDescription' => substr(preg_filter('/[^\p{L}0-9\s]+/u', '', $product->getDescription()), 0, 250)
            ];
        }
    ...
    /**
     * Get backlink. cancel and success urls.
     *
     * @param $orderId
     * @return string
     */
    private function getBackLink($orderId)
    {

        $cancelUrl  = $this->urlBuilder->getUrl('unipay/back/cancel');
        $successUrl = $this->urlBuilder->getUrl('unipay/back/result') . '?' . http_build_query([ 'order-id' => $orderId ]);

        return base64_encode($successUrl. "|" . $cancelUrl);
    }
    ...
    {
        return 'https://api.unipay.com/checkout/createorder';
    }
